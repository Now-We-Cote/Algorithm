...
const solution = (board, moves) => {
  const basketArr = []
  let result = 0;
  // accBoard 깡통 2차원 배열 만들어주기
  const accBoard = []
  board.forEach((arr) => {
    accBoard.push([])
  })

  // newBoard 인형뽑기 틀에서 세로로 한줄 단위로 위에있는 인형의 번호가 배열의 뒤에 오도록 정렬
  const newBoard = board.reduceRight((acc, cur, index) => {
  ...
  moves.forEach((index) => {
    // 꺼내온 인형의 번호 저장
    const dollNum = newBoard[index-1].pop

    // 바구니에 아무것도 없으면 바로 바구니에 저장
    // 바구니에 인형 있으면 지금 크레인에서 가져온 dollNum과 비교해서 같으면 result 1 증가하고 바구니에서 빼기
    // 바구니에 인형 있으면 지금 크레인에서 가져온 dollNum과 비교해서 다르면 바구니에 추가
    if (basketArr.length !== 0) {
      if(basketArr.at(-1) === dollNum) {
        result ++
        basketArr.pop()
      } else {
        basketArr.push(dollNum);
      }
    } else {
      basketArr.push(dollNum)
    }

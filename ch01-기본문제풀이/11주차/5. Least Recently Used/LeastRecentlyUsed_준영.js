 ...
 * 7 5 3 2 6
 */
const checkMemoryLengthExcess = (memory) => memory.length === 5 ? true : false
const checkExistJobNumber = (memory, jobNumber) => memory.includes(jobNumber) ? true : false;

console.time('time stamp: ')
const solution = (memoryLen, jobLen, jobArr) => {
  let memory = [];
  for (let job of jobArr) {
    if(checkExistJobNumber(memory, job)) {
      memory = memory.filter((jobNum) => jobNum !== job)
      memory.unshift(job);
    } else {
      if (checkMemoryLengthExcess(memory)) {
        memory.unshift(job)
        memory.splice(5, 1)
      } else {
        memory.unshift(job)

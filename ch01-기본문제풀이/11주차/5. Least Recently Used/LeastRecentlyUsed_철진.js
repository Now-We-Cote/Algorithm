...
// 문제 - 캐시메모리는 CPU와 주기억장치(DRAM) 사이의 고속의 임시 메모리로서 CPU가 처리할 작업 을 저장해 놓았다가 필요할 바로 사용해서 처리속도를 높이는 장치이다.
...
// 입력 예시 - 5,9, [1, 2, 3, 2, 6, 2, 3, 5, 7]

// 출력 예시 - [7, 5, 3, 2, 6]

{
  const solution = (S, N, works) => {
    const memory = Array(S).fill(0);

    for (work of works) {
      if (!memory.includes(work)) {
        memory.pop();
        memory.unshift(work);
      } else {

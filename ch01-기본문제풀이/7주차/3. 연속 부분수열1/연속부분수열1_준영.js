 ...
 *
 * 입력 예시 - 8, 6 [1, 2, 1, 3, 1, 1, 1, 2]
 *
 * 출력 예시 - 3
 */

const checkSum = (idx, sum, arr) => {
    let result = 0;
    while(result < sum && idx < arr.length) {
        result += arr[idx]
        idx++
    }
    if (result === sum) return true
    console.log(idx, sum, arr, result);
    return false
}

const solution = (num, sum, arr) => {
  return arr.filter((number, idx) => {
      return checkSum(idx, sum, arr)
  }).length
}

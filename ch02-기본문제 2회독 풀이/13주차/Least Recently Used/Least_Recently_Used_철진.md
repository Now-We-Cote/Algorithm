...
만약 캐시의 사이즈가 5이고 작업이 2 3 1 6 7 순으로저장되어있다면, (맨 앞이 가장 최근에 쓰인 작업이고, 맨 뒤는 가장 오랫동안 쓰이지 않은 작업이다.)
...
```javascript
const solution = (S, N, array) => {
  const memory = Array(S).fill(0);

  for (item of array) {
    if (!memory.includes(item)) {
      // 캐시 미스!
      memory.pop(); // 캐시 맨 뒤 작업 제거
      memory.unshift(item); // 맨 앞에 새 작업 추가
    } else {
      // 캐시 히트
      const index = memory.indexOf(item);
      memory.splice(index, 1); // 캐시에서 작업을 찾아서 빼내고
      memory.unshift(item); // 맨 앞에 추가
    }
  }
